...
class Beetle():
					...
					if packet_info != None:
						if packet_header == 'D':
							if self.name == "GUN":
								...
								print("DEBUG: PLAYER 1 SHOT RECEIVED ON LAPTOP ")
	...
	def send_ammo_update(self):
		if self.name == "GUN" and Globals.should_update_ammo:
			payload = self.encode_crc(DATA_HEADER + str(self.seq_num) + str(Globals.player1_ammo) + '0' * 13) 
			self.ch.write(payload)
			Globals.should_update_ammo = False

	def send_hp_update(self):
		if self.name == "VEST" and Globals.should_update_hp:
			hp_hundreds = (Globals.player1_hp // 100) % 10
			hp_tens = (Globals.player1_hp // 10) % 10
			hp_ones = Globals.player1_hp % 10
